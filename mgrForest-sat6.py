...
'''
train&test for sat-6 dataset
shell python mgrForest-sat7.py
need change './sat-6-full.mat' to your own path
...
class RFLayer_RAND(object):
    ...
    def fit(self, X_train, y_train, kfold=3, k=1, n_jobs=-1): # kfold = 5 yields 80/20 split, k will be the number of times we run validation
        if kfold > 1:
...
def main():
    m = loadmat('./sat-6-full.mat')
    X_train = m['train_x'].transpose(3,0,1,2)
    y_train = m['train_y'].transpose(1,0)
    y_train = np.array([np.argmax(one_hot)for one_hot in y_train])
    # print(data)
    X_test = m['test_x'].transpose(3,0,1,2)
    y_test = m['test_y'].transpose(1,0)
    ...
    print ('Statistics:')
    print ('The accuracy was:',accuracy_score(y_pred, y_test))
    print ('Params:')
